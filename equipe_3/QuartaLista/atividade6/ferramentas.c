...
{
  TPilhaEncDeFilas *pilha;
  pilha = (TPilhaEncDeFilas *)malloc(sizeof(TPilhaEncDeFilas));
  pilha->topo = 0;
  pilha->head = iniNoFila();
  return pilha;
}

int empilharFilaEnc(TFilaEncAlunos *fila, TPilhaEncDeFilas *pilha, char chave)
{
  PNoFila novo_no = iniNoFila();
  if(pilha != NULL && novo_no != NULL) 
  {
    PNoFila ultimo = pilha->head->esq;
...
int desempilharFilaEnc(TFilaEncAlunos *fila, TPilhaEncDeFilas *pilha)
{
  ...
  {
    PNoFila ultimo = pilha->head->esq;
    PNoFila penultimo = ultimo->esq;   
    penultimo->dir = pilha->head;
    pilha->head->esq = penultimo;

    fila = ultimo->fila;
    liberaFila(ultimo->fila);
    free(ultimo);
    pilha->topo--;
    return TRUE;
  }
...
void imprimePilha(TPilhaEncDeFilas *pilha)
{
  printf("Pilha\n");
  printf("[\n");
  PNoFila atual = pilha->head->dir;
  while (atual != pilha->head)
  {
    printf("    Turma %c\n", atual->chave);
    imprimeFila(atual->fila);
    atual = atual->dir;
  }
  printf(" ]\ntopo: %d\n", pilha->topo);
}
...
void liberaPilha(TPilhaEncDeFilas *pilha)
{
  ...
  {
    atual = atual->dir;
    liberaFila(atual->esq->fila);
    free(atual->esq);
  }
  liberaFila(atual->fila);
  free(atual);
}
...
TListAlunos *geraListaDeAlunos(int tam, int cap, int eOrd)
{
  ...
  for (int i = 0; i < tam;)
  {
    ...
    aluno.numMatricula +=
        (aluno.numMatricula == 202200000) ? 10000 : ((rand() % 2) + 1) * 10000;
