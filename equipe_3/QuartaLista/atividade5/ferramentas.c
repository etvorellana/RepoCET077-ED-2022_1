...
PNoPilha iniNoPilha(void)
{
    ...
    novo_no->dir = novo_no;
    novo_no->esq = novo_no;
    novo_no->pilha = iniPilhaEncDeFilas();
    return novo_no;
}
...
//Nova Fila
TFilaEncDePilhas *iniFilaAlunosEnc()
{
    TFilaEncDePilhas *fila = (TFilaEncDePilhas *)malloc(sizeof(TFilaEncDePilhas));
    fila->tam = 0;
    fila->head = iniNoPilha();
    return fila;
}

int insereNaFilaEnc(TPilhaEncAlunos *pilha, TFilaEncDePilhas *fila, char chave)
{
    NoFila *novo_no = iniNoPilha();
    if (fila != NULL && pilha != NULL)
    {
        NoFila *ultimo = fila->head->esq; // ultimo
        // sala de aula
        ...
        novo_no->pilha = pilha; // entrando os alunos
        fila->tam++;
        return TRUE;
    }        
    else
...
int removeDaFilaEnc(TPilhaEncAlunos *pilha, TFilaEncDePilhas *fila)
{
    if (fila->tam > 0)
    {
        NoFila *primeiro = fila->head->dir;
        NoFila *segundo = primeiro->dir;
...
{
    NoFila *atualFila = fila->head->dir;
    printf("Fila\n");
    printf("[\n");
...
{
    NoFila *atual = fila->head->dir;
    while (atual != fila->head)
    {
        atual = atual->dir;
        liberaPilha(atual->esq->pilha);
        free(atual->esq);
    }
    liberaPilha(atual->pilha);
    free(atual);
}
