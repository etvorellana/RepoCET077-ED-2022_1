...
int RemoverNoArvDeBusca(PRaizArvAluno raiz, int matricula) {
	if (raiz == NULL) {
		...
		PRaizArvAluno no_procurado = BuscarNoArvDeBusca(raiz, matricula);
    
		if (no_procurado != NULL) {
			if (no_procurado->dir == NULL && no_procurado->esq == NULL) { //Nó folha      
       free(no_procurado);//
       no_procurado=NULL;
        ...
        if (no_procurado->dir == NULL || no_procurado->esq == NULL) {
					...
					PRaizArvAluno aux = no_procurado->esq;
					while (aux->dir != NULL)
						aux = aux->dir;
            
          no_procurado->aluno = aux->aluno;
          aux->aluno.numMatricula = matricula;
          RemoverNoArvDeBusca(raiz->esq, matricula);
				}
      }
...
PRaizArvAluno BuscarNoArvBinaria(PRaizArvAluno raiz, int matricula) {
  ...
  if (raiz != NULL) 
    ...
    BuscarNoArvBinaria(raiz->esq, matricula);
    BuscarNoArvBinaria(raiz->dir, matricula);
	}
	return NULL;
}
