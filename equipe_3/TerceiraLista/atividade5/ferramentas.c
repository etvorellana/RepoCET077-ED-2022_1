...
void printLista(TAluno lista[], int tam)
{
	...
	for (int i = 0; i < tam; i++)
	{
		printf("[%d] %d, ",i, lista[i].numMatricula);
...
void quickSort(TAluno *lista, int inicio, int fim)
{
    if (inicio < fim)
    {
        TPilhaAlunos menoresQPivo, maioresQPivo;
        iniPilhaAlunos(&maioresQPivo, LSIZE);
        iniPilhaAlunos(&menoresQPivo, LSIZE);

        int pivo = lista[fim].numMatricula;
        int indicePivo = inicio; //Indice onde o pivo ficara fixo

        for (int i = inicio; i < fim; i++)
        {
            if (lista[i].numMatricula < pivo)
            {
                empilharAluno(lista[i], &menoresQPivo);
                indicePivo++;
            } 
            else 
                empilharAluno(lista[i], &maioresQPivo);
        }

        printf("Lista:\n");
        printLista(lista, LSIZE);
        printf("\nPIVO: %d\n", pivo);
        printf("Pilha menoresQPivo:\n");
        printLista(menoresQPivo.pilha, menoresQPivo.topo);
        printf("Pilha maioresQPivo:\n");
        printLista(maioresQPivo.pilha, maioresQPivo.topo);
        printf("\n");

        for (int i = inicio; i <= fim; i++)
        {
            if (i < indicePivo)
                desempilharALuno(&lista[i], &menoresQPivo);
            else if (i > indicePivo)
                desempilharALuno(&lista[i], &maioresQPivo);
            else
                lista[indicePivo] = lista[fim];
        }
        free(menoresQPivo.pilha);
        free(maioresQPivo.pilha);

        // Fara o sort na sublista da esquerda
        quickSort(lista, inicio, indicePivo - 1);
        // Fara o sort na sublista da direita
