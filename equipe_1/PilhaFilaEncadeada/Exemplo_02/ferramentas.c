...
int empilharAlunoEnc(TAluno aluno, TPilhaEncAlunos* pilha){
    ...
    PNoAluno aux = (PNoAluno)malloc(sizeof(PNoAluno)); //Aloco o espaço de um PNoAluno em uma variavel auxiliar do tipo PNoAluno
    ...
    aux->prox = pilha->topo; //Mo aponta pro topo da pilha
