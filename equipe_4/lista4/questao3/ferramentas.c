...
void NovaRandPilhaEnc(TPilhaEncAlunos *pilha, TAluno aluno, TListAlunos* registro){

     pilha->pilha = geraListaDeAlunos(LSIZE, LSIZE, TRUE);
     iniPilhaEncAlunos(pilha->pilha);

     int randowkey;
     registro = iniListAlunos(LSIZE, FALSE);
     randowkey = rand() % LSIZE;
     randowkey = pilha->pilha[randowkey].numMatricula;

    for(int i = 0; i < registro->tam; i++){
        empilharAlunoEnc(registro->lista[i], &pilha);
    }
    while(pilha->topo!=randowkey){
            desempilharAlunoEnc(&aluno, pilha->pilha);
            incAlunoNaLista(aluno, registro);
    }
    desempilharAlunoEnc(&aluno, pilha->pilha);

    for(int i = 0; i < registro->tam-1; i--){
        empilharAlunoEnc(registro->lista[i], &pilha->pilha);
    }
 }
