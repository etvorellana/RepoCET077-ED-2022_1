...
TListAlunos* geraListaDeAlunos(int tam, int cap, int eOrd)
{
    ...
    for (int i = 0; i < tam;)
    {
        ...
        aluno.numMatricula = (2000 + (rand() % 22+1)) * 100000;
...
void Acess_Biblioteca(int i, TListAlunos* lista, TFilaAlunos* fila){
    if(fila->fim+1 == fila->ini || (fila->fim+1 == LSIZE && !fila->ini)){
        printf("Fila Cheia\n");
        return;
    }
 fila->cap = i;
...
void Calouro_Pilha(TAluno aluno, TListAlunos* Lista, TPilhaAlunos* pilha, TFilaAlunos* fila){

    for(int i = 0; i < LSIZE; i++){
            if((Lista->lista[i].numMatricula / 10000) == 20221)
            empilharAluno(Lista->lista[i], pilha);
    else
        insereNaFila(Lista->lista[i], fila);
    }

    for(int i = 0; i < fila->cap; i++){
            removeDaFila(&aluno, fila);
            incAlunoNaLista(aluno, fila);
}
    for(int i = 0; i < pilha->topo; i++){
            desempilharALuno(&aluno, pilha);
            incAlunoNaLista(aluno, pilha);
}
