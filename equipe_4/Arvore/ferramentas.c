...
// NOVO
PRaizArvAlunoAVL criarArvoreDeLista_AVL(TListAlunos listaS)
{
...
PRaizArvAluno removerAlunoArvore_B(int numMatricula, PRaizArvAluno raiz)
{

	if (raiz == NULL)
		return NULL;

	if (numMatricula == raiz->aluno.numMatricula)
	{
		// SE NÃO TIVER DIREITA E ESQUERDA REMOVE
		...
		// SE TIVER SÓ DIREITA REMOVE A RAIZ E RETORNA A SUB-ARVORE DA DIREITA
		else if (raiz->esq == NULL)
		{
			PRaizArvAluno temp = raiz->dir;
			free(raiz);
			return temp;
		}

		// SE TIVER SÓ ESQUERDA REMOVE A RAIZ E RETORNA A SUB-ARVORE DA ESQUERDA
		else if (raiz->dir == NULL)
		{
			PRaizArvAluno temp = raiz->esq;
			free(raiz);
			return temp;
		}
		// SE TIVER OS DOIS LADOS, PEGA O NÓ MAIS PROFUNDO A PARTIR DA DIREITA E SUBSTITUI PELA RAIZ
		else
		{
			PRaizArvAluno dirMin = retornaMinimoDireita(raiz->dir);
			raiz->aluno = dirMin->aluno;
			raiz->dir = removerAlunoArvore_B(raiz->aluno.numMatricula, raiz->dir);
		}
	}
	else
	{
		raiz->dir = removerAlunoArvore_B(numMatricula, raiz->dir);
		raiz->esq = removerAlunoArvore_B(numMatricula, raiz->esq);
	}
	return raiz;
}

PRaizArvAluno removerAlunoArvore_BS(int numMatricula, PRaizArvAluno raiz)
{
	...
	if (numMatricula > raiz->aluno.numMatricula)
	{
		raiz->dir = removerAlunoArvore_BS(numMatricula, raiz->dir);
	}

	else if (numMatricula < raiz->aluno.numMatricula)
	{
		raiz->esq = removerAlunoArvore_BS(numMatricula, raiz->esq);
	}
	...
	else
	{
		...
		else
		{
			PRaizArvAluno dirMin = retornaMinimoDireita(raiz->dir);
			raiz->aluno = dirMin->aluno;
			raiz->dir = removerAlunoArvore_BS(raiz->aluno.numMatricula, raiz->dir);
		}
	}
...
PRaizArvAlunoAVL removerAlunoArvore_BAVL(int numMatricula, PRaizArvAlunoAVL raiz)
{

	if (raiz == NULL)
		return NULL;

	// SE A MATRICULA PROCURADA FOR MAIOR VAI PRA DIREITA
	if (raiz->aluno.numMatricula < numMatricula)
	{
		raiz->dir = removerAlunoArvore_BAVL(numMatricula, raiz->dir);
	}

	// SE A MATRICULA PROCURADA FOR MENOR VAI PRA ESQUERDA
	else if (raiz->aluno.numMatricula > numMatricula)
	{
		raiz->esq = removerAlunoArvore_BAVL(numMatricula, raiz->esq);
	}
	...
	else
	{
		...
		{
			PRaizArvAlunoAVL temp = raiz->dir;
			free(raiz);
			return temp;
		}
		...
		{
			PRaizArvAlunoAVL temp = raiz->esq;
			free(raiz);
			return temp;
		}
		...
		{
			PRaizArvAlunoAVL dirMin = retornaMinimoAVL(raiz->dir);
			raiz->aluno = dirMin->aluno;
			raiz->dir = removerAlunoArvore_BAVL(raiz->aluno.numMatricula, raiz->dir);
		}
	}

	// PEGA A ALTURA DO NÓ DA ESQUERDA E DA DIREITA
...
PRaizArvAluno retornaMinimoDireita(PRaizArvAluno *raiz)
{
	PRaizArvAluno temp = raiz;
...
PRaizArvAlunoAVL retornaMinimoAVL(PRaizArvAlunoAVL *raiz)
{
	PRaizArvAlunoAVL temp = raiz;
...
int buscarAlunoArvore_B(int numMatricula, PRaizArvAluno raiz)
{
	if (raiz == NULL)
		return FALSE;
	...
	// SE ACHAR NO NÓ DA ESQUERDA TERMINA
	int result = buscarAlunoArvore_B(numMatricula, raiz->esq);

	if (result)
		return TRUE;

	// SE NÃO PROCURA NA DIREITA
	return buscarAlunoArvore_B(numMatricula, raiz->dir);
}

int buscarAlunoArvore_BS(int numMatricula, PRaizArvAluno raiz)
{
	if (raiz == NULL)
		return FALSE;
	...
	if (numMatricula > raiz->aluno.numMatricula)
	{
		return buscarAlunoArvore_BS(numMatricula, raiz->dir);
	}
	// SE A MATRICULA PROCURADA FOR MENOR VAI PRA ESQUERDA
	else
	{
		return buscarAlunoArvore_BS(numMatricula, raiz->esq);
	}
}

int buscarAlunoArvore_BAVL(int numMatricula, PRaizArvAlunoAVL raiz)
{
	if (raiz == NULL)
		return FALSE;
