...
int remAlunoPilha(TPilhaAlunos *pilha, int chave){
  TPilhaAlunos aux;
  TAluno trnsf;

  iniPilhaAlunos(&aux, pilha->cap);

  do{
    desempilharALuno(&trnsf, pilha);
    if(trnsf.numMatricula!=chave){
      empilharAluno(trnsf,&aux);
    }
  }while(pilha->topo>0 && trnsf.numMatricula!=chave);

  if(trnsf.numMatricula!=chave && pilha->topo==0)
    return 0;

  while(aux.topo>0){
    desempilharALuno(&trnsf, &aux);
    empilharAluno(trnsf, pilha);
  }
  return 1;
}
...
void printFila(TAluno fila[], int inicio, int fim){
  printf("[ \n ");
	for (int i = inicio; i < fim; i++)
	{
		printf("%d, ", fila[i].numMatricula);
...
TAluno geraAluno()
{
  ...
  aluno.numMatricula = (PANO + (random() % (UANO-PANO)+1)) * 100000;
